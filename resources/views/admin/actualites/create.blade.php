...
<div class="bg-white rounded-lg shadow-sm border border-black/5 p-6">
    <h3 class="font-bold text-lg mb-4">Ajouter une nouvelle actualité</h3>
    ...
    <form action="{{ route('admin.actualites.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <select name="categorie_id" id="categorie_id" class="mt-1 block w-full px-3 py-2 border border-black/10 rounded-md shadow-sm focus:outline-none focus:ring-rdcBlue focus:border-rdcBlue sm:text-sm" required>
                    ...
                    @foreach($categories as $categorie)
                        ...
                        <option value="{{ $categorie->id }}" @if(old('categorie_id') == $categorie->id) selected @endif>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div>
        ...
        <div>
            ...
            <input type="date" name="published_at" id="published_at" class="mt-1 block w-full px-3 py-2 border border-black/10 rounded-md shadow-sm focus:outline-none focus:ring-rdcBlue focus:border-rdcBlue sm:text-sm">
        </div>
